  ...
  <!-- MODAL  -->
  <div class="modal fade" id="nueva_moneda" tabindex="-1" role="dialog" aria-labelledby="nueva_moneda_label" aria-hidden="true">
      <!-- MODAL  -->
  ...
  <script>
      function cargar_tabla_monedas() {
          vtabla = $('#style-3').DataTable({
              ...
              "stripeClasses": [],
              'ajax': {
                  'url': '<?php echo ENLACE_WEB; ?>mod_diccionario_moneda/ajax/listado_diccionario_moneda_ajax.php',
                  'Method': 'POST'
              },
              retrieve: true,
              deferRender: true,
              ...
              'columns': [{
                      ...
                      render: function(data, type, row) {
                          ...
                          return `<a onclick="ver_moneda(${row.ID});" href="#">${row.codigo}</a>`
                      }
                  },
                  {
                      data: 'simbolo',
                      searchable: true,
                      createdCell: function(td, cellData, rowData, row, col) {
                          $(td).addClass('text-center').attr('data-label', 'simbolo');
                      },
                      render: function(data, type, row) {

                          return `<a onclick="ver_moneda(${row.ID});" href="#">${row.simbolo}</a>`
                      }
                  },
                  {
                      data: 'etiqueta',
                      ...
                      render: function(data, type, row) {
                          return `<a onclick="ver_moneda(${row.ID});" href="#">${data}</a>`
                      }
  ...
  <script>
      $(document).ready(function() {
          ...
          let newButton = $('<button>')
              .html('<i class="fa-solid fa-plus"></i> Nueva moneda')
  ...
  <script type="text/javascript">
      ...
      function ver_moneda(int = null) {
          ...
          $.ajax({
              ...
              $("#nueva_moneda").html(html).modal('show');
          });

      }


      function actualizar_moneda(int) {
          ...
          $.ajax({
              ...
              if (mensaje.exito === 1) {
                  add_notification({
                      text: 'Moneda actualizado exitosamente',
                      actionTextColor: '#fff',
                      backgroundColor: '#00ab55',
                      dismissText: 'Cerrar'
                  });
      ...
      function borrar_moneda($id) {
          ...
          var message = "Está seguro(a) que desea eliminar la Moneda? ";
          ...
          add_notification({
              ...
              onActionClick: function(element) {
                  ...
                  $.ajax({
                      ...
                      if (data.exito == 1) {
                          add_notification({
                              text: 'Moneda Eliminado exitosamente',
                              pos: 'top-right',
                              actionTextColor: '#fff',
                              backgroundColor: '#00ab55'
                          });
