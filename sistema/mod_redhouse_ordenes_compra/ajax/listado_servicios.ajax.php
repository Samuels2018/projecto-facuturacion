<?php
    ...
    $rowid = $_REQUEST['fiche'];
    // WHERE 
    $where ='';
    $where .=' WHERE  sc.fk_orden = '.$rowid.' ';
    ...
    $sql= "SELECT
                ...
                FROM    a_medida_redhouse_orden_compra_servicios   sc 
...
$montos_subtotales = 0;
$montos_totales = 0;
$moneda_simbolo = $orden->moneda_simbolo;
...
while ($data = $db->fetch(PDO::FETCH_OBJ)):
    ...
    $tr.=
        '<tr class="ng-scope '. ( ($_REQUEST['actividad']==$data->rowid) ? 'shake-row' :'' ).'"  style="cursor:pointer;">
            ...
            <td title="'.ucwords($data->label).'">'.returnSplitNameClient(ucwords($data->label),25).'</td>
            ...
            {
                $tr.='<td>'.$aa.' <i class="fa fa-fw fa-edit" aria-hidden="true"></i> '.$aa2.' <button type="button"  class="boton-servicio" style="border:none; background-color:transparent;"   onclick="eliminarServicio('.$data->rowid.')"><i class="fa fa-trash-o" aria-hidden="true"></i></button></td>';
            }            
...
if ($tr == '') {
    echo '<tr><td colspan="12" style="text-align:center">No se han encontrado Servicios/Productos en esta Orden de compra </td></tr>';
