...
class Validez_oferta extends  Seguridad
{
    ...
    {	
       ...
       $sql="SELECT  u.* 
                    FROM diccionario_validez_oferta u     
	...
	{
	    try {
	        // Preparar la consulta SQL para insertar en la tabla 'diccionario_validez_oferta'
	        $sqlcheck = "
	            INSERT INTO diccionario_validez_oferta (label, entidad, creado_fecha, creado_fk_usuario, activo)
...
{
    try {
        // Preparar la consulta SQL para actualizar la tabla 'diccionario_validez_oferta'
        $sqlupdate = "
            UPDATE diccionario_validez_oferta
...
{
    // Consulta SQL para borrar en la tabla 'diccionario_validez_oferta'
    $sql_diccionario_validez_oferta = "UPDATE diccionario_validez_oferta SET borrado = 1,
    ...
    try {
        ...
        // Actualización en la tabla diccionario_validez_oferta
        $update_stmt = $this->db->prepare($sql_diccionario_validez_oferta);
        ...
        if ($update_stmt->errorCode() !== '00000') {
            $this->sql = $sql_diccionario_validez_oferta;
            $this->error = implode(", ", $update_stmt->errorInfo());
        }
