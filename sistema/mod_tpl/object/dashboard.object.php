...
class Dashboard extends  Seguridad
{
        ...
        switch ($filtro) {
                ...
                */
                $sql = "WITH ultimos_anios AS (
                    ...
                    LEFT JOIN fi_europa_facturas f
                        ...
                        AND f.estado = 1
                        AND f.entidad = :entidad
                    LEFT JOIN fi_europa_documentos_pagos fp
                        ON YEAR(fp.fecha_pago) = ua.anio AND fp.fk_documento = f.rowid
                    GROUP BY 
                        ua.anio
                    ORDER BY 
                        ua.anio DESC;
            ...
            case "mensual": // Mostrar 12 meses
                $sql = "WITH ultimos_meses AS (
                    ...
                    FORMAT(IFNULL(( 
                            SELECT SUM(fp.monto) 
                            FROM fi_europa_documentos_pagos fp 
                            WHERE fp.fk_documento IN 
            ...
            case "semanal": // Mostrar 12 semanas
                $sql = "WITH ultimas_semanas AS (
                    ...
                    LEFT JOIN fi_europa_facturas f
                        ...
                        AND f.entidad = :entidad
                    LEFT JOIN fi_europa_documentos_pagos fp
                        ON YEARWEEK(fp.fecha_pago, 3) = us.semana AND fp.fk_documento = f.rowid
                    GROUP BY 
                        us.semana
                    ORDER BY 
                        us.semana;
                ...
                */
                $sql = "WITH ultimos_dias AS (
                    ...
                    LEFT JOIN fi_europa_facturas f
                        ...
                        AND f.entidad = :entidad
                    LEFT JOIN fi_europa_documentos_pagos fp
                        ON DATE(fp.fecha_pago) = ud.fecha AND fp.fk_documento = f.rowid
                    GROUP BY 
                        ud.fecha
                    ORDER BY 
                        ud.fecha;
    ...
    public function devolver_productos_mas_vendidos($max = 10)
    {
        $sql = "SELECT 
                ...
                INNER JOIN fi_europa_facturas_detalle fd ON f.rowid = fd.fk_documento
    ...
    public function devolver_datos_dashboard_ultimas_facturas($max = 10)
    {
        ...
        INNER JOIN 
            fi_europa_facturas_detalle fd ON f.rowid = fd.fk_documento
        INNER JOIN 
