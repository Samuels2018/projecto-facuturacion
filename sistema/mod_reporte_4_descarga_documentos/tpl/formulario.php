...
<script type="text/javascript">
  ...
  function cargarGrilla(column, value, mes, category) {
    const ajaxoption = {
      url: '<?php echo ENLACE_WEB; ?>mod_reporte_4_descarga_documentos/ajax/detalle.ajax.php',
      type: 'POST',
      data: {
        column: column,
        value: value,
  ...
  function drawChart() {
    ...
    var options = {
      ...
      colors: ['<?php echo $Reporte->color_soportado; ?>', '<?php echo $Reporte->color_repercutido; ?>', '<?php echo $Reporte->color_a_pagar; ?>']
    ...
    google.visualization.events.addListener(chart, 'select', function() {
      ...
      if (selection.length > 0) {
        ...
        let analizando_txt = '';
        if (category == "Repercutido") {
          analizando_txt = "Analizando Facturas Ventas ";
        } else if (category == "Soporte") {
          analizando_txt = "Analizando Facturas Compras ";
        }
        cargarGrilla(column, value, data.getValue(row, 0), category)

        $('#detalle').prepend(`<span>${analizando_txt}. </span><span>Mostrando Mes: ${data.getValue(row, 0)}`)
      }
    });
  }
