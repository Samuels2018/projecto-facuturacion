...
class Forma_pago extends  Seguridad
{
    ...
    function fetch($id)
    {
        ...
        $sql = "SELECT  u.* 
                    FROM diccionario_formas_pago u
        ...
        $this->importes_iguales        = $u['importes_iguales'];
        $this->ultimo_dia        = $u['ultimo_dia'];
        $this->activo = $u['activo'];

        $sql_detalle = "SELECT d.* FROM diccionario_formas_pago_detalle d 
        WHERE d.fk_formapago = :rowid AND d.activo= 1";
        $dbDet = $this->db->prepare($sql_detalle);
        $dbDet->bindValue(':rowid', $id, PDO::PARAM_STR);
        $dbDet->execute();
        $fetchDet = $dbDet->fetchAll();
        $this->detalle = $fetchDet;
    }
    ...
    public function crear_forma_pago()
    {
        try {
            // Preparar la consulta SQL para insertar en la tabla 'diccionario_formas_pago'
            $sqlcheck = "
	            INSERT INTO diccionario_formas_pago (label, importes_iguales, ultimo_dia, entidad, creado_fecha, creado_fk_usuario, activo)
	            VALUES (:label, :importes_iguales, :ultimo_dia, :entidad, now(), :creado_fk_usuario, :activo)
	        ";
            $insert_stmt = $this->db->prepare($sqlcheck);

            // Asignar valores a los parámetros
            $insert_stmt->bindValue(':label', $this->label);
            $insert_stmt->bindValue(':importes_iguales', $this->importes_iguales);
            $insert_stmt->bindValue(':ultimo_dia', $this->ultimo_dia);
            ...
            if ($insert_stmt->execute()) {
                ...
                return ['exito' => 1, 'mensaje' => 'Registro insertado correctamente', 'rowid' => $ultimo_id];
            } else {
                // Manejo de error
                $this->sql = $sqlcheck;
    ...
    {
        try {
            // Preparar la consulta SQL para actualizar la tabla 'diccionario_formas_pago'
            $sqlupdate = "
                UPDATE diccionario_formas_pago
                SET label = :label, activo = :activo,
                    ultimo_dia = :ultimo_dia, importes_iguales = :importes_iguales
            ...
            $update_stmt->bindValue(':importes_iguales', $this->importes_iguales);
            $update_stmt->bindValue(':ultimo_dia', $this->ultimo_dia);
    ...
    public function borrar_forma_pago($id)
    {
        // Consulta SQL para borrar en la tabla 'diccionario_formas_pago'
        $sql_diccionario_formas_pago = "UPDATE diccionario_formas_pago SET borrado = 1,
        ...
        try {
            ...
            // Actualización en la tabla diccionario_formas_pago
            $update_stmt = $this->db->prepare($sql_diccionario_formas_pago);
