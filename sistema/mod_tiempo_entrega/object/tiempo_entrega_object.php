...
class Tiempo_entrega extends  Seguridad
{
    ...
    {	
       ...
       $sql="SELECT  u.* 
                    FROM diccionario_tiempo_entrega u     
    ...
    public function listar()
    {
        $sql = "SELECT u.* 
                FROM diccionario_tiempo_entrega u     
                WHERE entidad = :entidad AND activo = 1  AND u.borrado = 0 ORDER BY CAST(u.label AS UNSIGNED) ASC";
	...
	{
	    try {
	        // Preparar la consulta SQL para insertar en la tabla 'diccionario_tiempo_entrega'
	        $sqlcheck = "
	            INSERT INTO diccionario_tiempo_entrega (label, entidad, creado_fecha, creado_fk_usuario, activo)
...
{
    try {
        // Preparar la consulta SQL para actualizar la tabla 'diccionario_tiempo_entrega'
        $sqlupdate = "
            UPDATE diccionario_tiempo_entrega
...
{
    // Consulta SQL para borrar en la tabla 'diccionario_tiempo_entrega'
    $sql_diccionario_tiempo_entrega = "UPDATE diccionario_tiempo_entrega SET borrado = 1,
    ...
    try {
        ...
        // Actualización en la tabla diccionario_tiempo_entrega
        $update_stmt = $this->db->prepare($sql_diccionario_tiempo_entrega);
        ...
        if ($update_stmt->errorCode() !== '00000') {
            $this->sql = $sql_diccionario_tiempo_entrega;
            $this->error = implode(", ", $update_stmt->errorInfo());
        }
