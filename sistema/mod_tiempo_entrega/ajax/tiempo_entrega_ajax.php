...
if (!empty($_POST['action'])) :
  ...
  $obj = new Tiempo_entrega($dbh,$_SESSION['Entidad']);
...
if (!empty($_POST['id'])) {
  ...
  switch ($_POST['action']):
    ...
    case 'guardar':
      ...
      if(intval($id)>0)
      {
        if (encontrar_duplicado('diccionario_tiempo_entrega', 'label', $_POST['label'], $_SESSION['Entidad'], $id)['total'] > 0) {
        ...
        if (encontrar_duplicado('diccionario_tiempo_entrega', 'label', $_POST['label'], $_SESSION['Entidad'])['total'] > 0) {
          echo json_encode(['exito' => 0, 'mensaje' => 'Esta etiqueta ya existe']);
          exit;
        }
