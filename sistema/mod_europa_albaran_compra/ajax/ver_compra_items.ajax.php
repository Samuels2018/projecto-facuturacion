...
//  Objeto 
//
require_once ENLACE_SERVIDOR . 'mod_europa_albaran_compra/object/Albaran_compra.object.php';
...
$Documento2  = new Albaran_compra($dbh, $_SESSION['Entidad']);
$datos_tooltip = [];
if ($document_id != '') {
...
if (
    ...
    if (!empty($_POST['linea'])) {
        ...
        $Documento2->actualizar_detalle(true);
        $Documento2->fetch($document_id);
    } else {
...
$sql = "SELECT 
        ...
        m.destino_documento ,
        m.origen_cantidad   ,
        compras.referencia as compra_referencia   ,
        compras.rowid      as compra_rowid        ,
        d.etiqueta as compra_etiqueta             ,
        d.class    as compra_class                ,
        p.stock as stock
    
    
    FROM  " . $Documento2->documento_detalle . "   f
    LEFT JOIN fi_productos p ON p.rowid = f.fk_producto 
    LEFT join fi_europa_documentos_movimientos_detalles  m on m.origen_documento    = '{$Documento2->documento}' and  origen_fk_documento = f.fk_documento   and f.origen_fk_documento_detalle =  f.rowid 
    LEFT join fi_europa_compras         compras      on compras.rowid  = m.destino_fk_documento
    LEFT JOIN " . DB_NAME_UTILIDADES_APOYO . ".diccionario_albarenes_compra_europa_diccionario d ON compras.estado = d.rowid


    WHERE IFNULL(m.borrado,0) = 0 AND f.fk_documento = ?
    GROUP BY f.rowid
    ORDER BY 
    CASE 
...
while ($obj = $db->fetch(PDO::FETCH_ASSOC)) {
     ...
     $filterdata = []; $html_compras = ''; $html_albaran = ''; $total_albaran = 0; $sum_det = 0; $html_tooltip = '';
     if(count($datos_tooltip)>0){
         ...
         if(count($filterdata)>0 ){
            $total_albaran = floatval($filterdata[0]["cantidad_cab"]);
            $total_albaran_mostrar = numero_simple($filterdata[0]["cantidad_cab"]);
            $html_albaran = "<tr><td>{$datos_tooltip[0]["referencia_cab"]}</td><td>{$total_albaran_mostrar}</td></tr>";
    
            foreach ($filterdata as $key => $value) {
                $sum_det += floatval($value["cantidad_det"]);
                $cantidad_det_mostrar = numero_simple($value["cantidad_det"]);
                $html_compras .= "<tr><td>{$value["referencia_det"]}</td><td>-{$cantidad_det_mostrar}</td></tr>";
            }
            $total_albaran = $total_albaran-$sum_det;
            $total_albaran_mostrar = numero_simple($total_albaran);
            $html_tooltip = "<table>".$html_albaran. $html_compras."<tr><td><b>Disponible Albarán</b></td><td><b>{$total_albaran_mostrar}</b></td></tr></table>";
        }
        $filterdata = [];
     }
     /* Construyo la tabla tooltip para cada linea */
   ...
   <input ' . $disable_input . ' readonly="" type="hidden"  name="line_subtotal[]" value="' . numero_simple($obj['subtotal']) . '" class="form-control " id="subtotal_' . $obj['rowid'] . '">';
    ...
    $tr .= "
                <td $style_centrado align='left' width='5%' >" . $input_cantidad . "<label class='oculto_' id='label_cantidad_" . $obj['rowid'] . "'>" . numero_simple($obj['cantidad']) . "</label>
                </td>
    ...
    if ($Documento2->estado > 0) {
        $tr .= "<td>";
        $tr .= "<a href='" . ENLACE_WEB . "compra/" . $obj['compra_rowid'] . "'>
                        <span class='badge badge-light-" . $obj['compra_class'] . "' >{$obj['compra_referencia']}</span>
                    </a> " . $obj['origen_cantidad'];
        $tr .= "</td>";
    }
    $tr .= "</tr>";
}
