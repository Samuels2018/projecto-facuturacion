...
$sqlstr = "SELECT 
    ...
    (select COUNT(rowid) 
        ...
        where origen_documento = 'fi_europa_albaranes_compras' and destino_documento = 'fi_europa_compras' 
        and origen_fk_documento = a.rowid
        and IFNULL(borrado,0) = 0
    ) as total_facturas_compras,
    (SELECT CONCAT(fi_europa_compras.referencia,'***', fi_europa_compras.rowid ) 
        FROM fi_europa_documentos_movimientos
        left join fi_europa_compras on fi_europa_compras.rowid = fi_europa_documentos_movimientos.destino_fk_documento 
        WHERE fi_europa_documentos_movimientos.origen_documento = 'fi_europa_albaranes_compras' 
        AND destino_documento = 'fi_europa_compras' 
        AND fi_europa_documentos_movimientos.origen_fk_documento = a.rowid 
        AND IFNULL(fi_europa_documentos_movimientos.borrado,0) = 0
        LIMIT 1) AS factura_compra_unica,
    CASE 
        WHEN t.tipo = 'fisica' THEN CONCAT(t.nombre, ' ', t.apellidos) 
        ELSE t.nombre  
    END AS cliente_tercero 
FROM fi_europa_albaranes_compras a
LEFT JOIN fi_usuarios u ON a.fk_usuario_crear = u.rowid
LEFT JOIN fi_terceros t ON a.fk_tercero = t.rowid
LEFT JOIN ".DB_NAME_UTILIDADES_APOYO.".diccionario_albarenes_compra_europa_diccionario d ON a.estado = d.rowid
...
$sqlstrTotal = "SELECT COUNT(*) FROM fi_europa_albaranes_compras a WHERE a.entidad = :entidad";
...
if (isset($array_columnas['fecha_vencimiento']) && $array_columnas['fecha_vencimiento'] != '') {
    $sqlstrCountFiltered .= " AND a.fecha_vencimiento BETWEEN :fecha_inicio_venc AND :fecha_fin_venc";
}
...
if (isset($fecha_inicio_venc) && isset($fecha_fin_venc)) {
    $countFilteredStmt->bindParam(':fecha_inicio_venc', $fecha_inicio_venc, PDO::PARAM_STR);
    $countFilteredStmt->bindParam(':fecha_fin_venc', $fecha_fin_venc, PDO::PARAM_STR);
}
...
if (isset($fecha_inicio_venc) && isset($fecha_fin_venc)) {
    $dataB->bindParam(':fecha_inicio_venc', $fecha_inicio_venc, PDO::PARAM_STR);
    $dataB->bindParam(':fecha_fin_venc', $fecha_fin_venc, PDO::PARAM_STR);
}
...
foreach ($Records as $row) {
    ...
    $fileExists = false; // Aquí deberías implementar tu lógica para verificar archivos
    ...
    $data[] = array(
        ...
        "estado" => $row['estado'],
        "total_facturas_compras" => (int)$row['total_facturas_compras'],
        "factura_compra_unica_id" => isset($datos[1]) ? $datos[1] : '',
        "factura_compra_unica_ref" => isset($datos[0]) ? $datos[0] : '',
