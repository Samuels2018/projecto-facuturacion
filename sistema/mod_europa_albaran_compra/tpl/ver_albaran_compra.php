...
$mensaje_javascript = [];
$id = $_GET['fiche'];
$Documento->id = $_GET['fiche'];
$Documento->fetch($id);

$tipo = $Documento->nombre_clase;

$Documento->obtener_documentos_destino();

foreach ($Documento->movimiento_destinos as $key =>  $item) {
   $documentos_destinos_string .= '<a href="' . ENLACE_WEB . 'compra/' . $item["rowid"] . '" ><span class="p-2 badge badge-light-success"> ' . $item["referencia"] . '</span></a>';
}

if ($_GET['accion'] == "validar_albaran" and $Documento->estado == 0) {
   ...
   if ($Documento->movimiento_origen == '') {
      ...
      $Bodega->motivo         = "Albaran Compra";
      $Bodega->mover_bodega_en_compra($Documento, $_SESSION['usuario'], $_GET['option_bodega']);
   }
}

// if ($_GET['accion'] == "anular_albaran_compra" and $Documento->estado == 1) {
//    $motivoCodificado  = urldecode($_GET['motivo']);
...
// }
// $estados_permitidos_cancelar = [1, 4];
// if ($_GET['accion'] == "cancelar_albaran_compra" and in_array($Documento->estado, $estados_permitidos_cancelar)) {
//    $Documento->cambiar_estado(5);
//    $Bodega = new Bodegas($dbh, $_SESSION['Entidad']);
//    $Bodega->usuario = $_SESSION["usuario"];
//    $Bodega->documento_tipo = "albaran_compra";
//    $Bodega->motivo         = "Cancelación Albaran Compra";
//    $Bodega->recuperar_bodega_en_compra($Documento, $_SESSION['usuario'], 'disminuir');
...
<div class="middle-content container-xxl p-0">
   <div class="page-meta mb-4">
      <nav class="breadcrumb-style-one" aria-label="breadcrumb">
         <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page"><a href="<?php echo ENLACE_WEB; ?>albaranes_listado"><?php echo $Documento->documento_txt['singular']; ?></a></li>
   ...
   <div class="content" id="formulario_factura">
         ...
         <div class="col-md-7">
            <div class="card">
               <div class="card-body">
                  ...
                  <div class="row alert alert-outline-primary alert-dismissible fade show mb-4" role="alert" id="muestra_opciones">
            ...
            <div class="card">
               <div class="card-body">
                  <div class="form-group row">
                     <div class="col-sm-6">
                        <h3><a href="<?php echo ENLACE_WEB; ?>albaran_compra/<?php echo $Documento->id; ?>"><span><?php echo $Documento->referencia; ?></span></a></h3>
                     </div>
                  </div>
                  <?php if ($Documento->estado == 0) { ?>
                  ...
                  <?php if ($Documento->movimiento_destinos != null) { ?>
                     <div class="form-group row">
                        <label for="forma_pago" class="col-sm-6 col-form-label col-form-label-sm">Documentos generados</label>
                        <div class="col-sm-6" id="documentos_destino">
                           <?php echo $documentos_destinos_string; ?>
                        </div>
                     </div>
                  <?php } ?>
      ...
      <div class="row mt-3 zona_trabajo_factura ">
         ...
         <div class="row mt-5">
            <div class="col-xs-12">
               <a href="<?php echo ENLACE_WEB; ?>albaranes_listado" class="btn btn-outline-primary _effect--ripple waves-effect waves-light">
               ...
               <?php if ($Documento->id  > 0) { ?>
                  <div class="btn-group" role="group">
                     ...
                     <ul id="dropdown_opciones_avanzadas" class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                        ...
                        <!-- <?php //if ($Documento->estado ==  1 || $Documento->estado ==  4) {  
                              ?>
                           <li>
                  ...
                  <?php if ($Documento->estado == 0) {   ?>
                     <a href="#" onclick="validar_albaran()" class="btn btn-outline-primary _effect--ripple waves-effect">
                        <i class="fas fa-check"></i>
                        Aprobar
                     </a>
                  <?php } ?>

                  <!-- Aprobado y Entrega parcial -->
                  <?php if ($Documento->estado == 1 || $Documento->estado == 4) {   ?>
                     <span id="facturar_albaran">
                        <a href="#" onclick="facturar_albaran()" class="btn btn-outline-primary _effect--ripple waves-effect">
                           <i class="fas fa-check"></i>
                           Facturar Albarán <?php echo $Documento->referencia; ?>
                        </a>
                        <span></span>
                     </span>
                  <?php } ?>
...
<script>
   ...
   $(document).ready(function() {
      ...
      // Captura el evento si ya ocurrió
      if (window.lastClienteModalEvent) {
         window.dispatchEvent(window.lastClienteModalEvent);
      }
   ...
   function validar_albaran() {
      ...
      Swal.fire({
         ...
         html: `
               ¿Deseas aprobar el documento <?php $Documento->referencia; ?> ?<br>
         ...
         if (result.isConfirmed) {
            window.location.href = "<?php echo ENLACE_WEB; ?>dashboard.php?&accion=validar_albaran&fiche=<?php echo $Documento->id; ?>&option_serie=" + codigo_serie_selected + "&option_bodega=" + codigo_bodega_selected;
   ...
   function facturar_albaran() {
      Swal.fire({
         title: 'Confirmar facturar',
         text: "¿Seguro que deseas facturar el Albarán: <?php echo $Documento->referencia ?>?",
         icon: 'warning',
         showCancelButton: true,
         ...
         if (result.isConfirmed) {
            ...
            $.ajax({
                  method: "POST",
                  url: "<?php echo ENLACE_WEB; ?>mod_europa_albaran_compra/class/albaranes_compra.class.json.php",
                  data: {

                     documento: '<?php echo $Documento->id; ?>',
                     tipo: '<?php echo $Documento->nombre; ?>',
                     'accion': 'ligar_documento'
                  },
               ...
               .done(function(msg) {
                  ...
                  if (data.exito == 1) {
                     ...
                     if ("location" in data) {
                        window.open('<?php echo ENLACE_WEB; ?>' + data.location, '_blank');
   ...
   function anular_documento() {
      htmlTextMotivo = '<textarea id="txtMotivoAnular" rows="2" maxlength="100" class="form-control"></textarea>';

      Swal.fire({
         title: 'Anular <?php echo $Documento->referencia; ?>',
         html: `
         Esta acción sobre el documento <?php echo $Documento->documento_txt['singular']; ?> es irrevocable!!  <br/><br/>
         Debe ingresar un <b>motivo de Anulación</b> para continuar. <br> ${htmlTextMotivo}
         `,
         icon: 'warning',
         showCancelButton: true,
         ...
         reverseButtons: true,
         preConfirm: () => {
            const textMotivo = $('#txtMotivoAnular').val()
            if (textMotivo == '') {
               $('#txtMotivoAnular').addClass('input_error')
               Swal.showValidationMessage('Por favor, ingrese el Motivo de Anulación');
               return false;
            } else {
               $('#txtMotivoAnular').removeClass('input_error')
            }
         }
      }).then((result) => {
         if (result.isConfirmed) {

            const sanitizedData = DOMPurify.sanitize($('#txtMotivoAnular').val());
            $.ajax({
                  method: "POST",
                  url: "<?php echo ENLACE_WEB; ?>mod_europa_albaran_compra/class/albaranes_compra.class.json.php",
                  data: {
                     documento: '<?php echo $Documento->id; ?>',
                     motivo: 'ANULACIÓN: ' + encodeURIComponent(sanitizedData),
                     'accion': 'anular_albaran_compra'
                  },
                  beforeSend: function(xhr) {
                     // Opcional: mostrar un loader o mensaje
                  }
               ...
               .done(function(msg) {
                  ...
                  if (data.exito == 1) {
                     ...
                     window.location.href = '<?php echo ENLACE_WEB; ?>ver_albaran/<?php echo $Documento->id; ?>';
            ...
            // window.location.href = "<?php echo ENLACE_WEB; ?>anular_albaran_compra/<?php echo $Documento->id; ?>/" + encodeURIComponent(sanitizedData);
         } else if (result.dismiss === Swal.DismissReason.cancel) {
            // Acciones a realizar si se cancela
         }
      });
   ...
   function cancelar_documento() {
      Swal.fire({
         ...
         if (result.isConfirmed) {
            window.location.href = "<?php echo ENLACE_WEB; ?>cancelar_albaran_compra/<?php echo $Documento->id; ?>";
         } else if (result.dismiss === Swal.DismissReason.cancel) {
            // Acciones a realizar si se cancela
         }
      });
