...
if ($_GET['accion'] == "validar_albaran_venta" and $Documento->estado == 0) {
   ...
   $Bodega->documento_tipo = "venta_albaran";
   $Bodega->motivo         = "Albarán Venta";
   $Bodega->mover_bodega_en_venta($Documento, $_SESSION['usuario'], $_GET['option_bodega']);
}
...
<div class="middle-content container-xxl p-0">
   <div class="page-meta mb-4">
      <nav class="breadcrumb-style-one" aria-label="breadcrumb">
         <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page"><a href="<?php echo ENLACE_WEB; ?>albaran_venta_listado"><?php echo $Documento->documento_txt['singular']; ?></a></li>
   ...
   <div class="content" id="formulario_factura">
            ...
            <div class="card">
               <div class="card-body">
                  <div class="form-group row">
                     <div class="col-sm-6">
                        <h3><a href="<?php echo ENLACE_WEB . $Documento->ver_url; ?>/<?php echo $Documento->id; ?>"><span><?php echo $Documento->referencia; ?></span></a></h3>
                     </div>
                  </div>
                  <?php if ($Documento->estado == 0) { ?>
      ...
      <div class="row mt-3 zona_trabajo_factura ">
         ...
         <div class="row mt-5">
            <div class="col-xs-12">
               <a href="<?php echo ENLACE_WEB; ?>albaran_venta_listado" class="btn btn-outline-primary _effect--ripple waves-effect waves-light">
                     ...
                     <ul id="dropdown_opciones_avanzadas" class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                        ...
                        <?php if ($Documento->estado  != 0) { ?>
                           ...
                           <?php if (file_exists($filePath)) { ?>
                              <li> <a class="dropdown-item" href="<?php echo ENLACE_WEB; ?>mod_documentos_mercantiles/ajax/descargar.comprobante.php?tipo=<?php echo $Documento->nombre_clase ?>&id=<?php echo $Documento->id; ?>">Descargar <?php echo $Documento->referencia; ?> - XML </a></li>
                              </li>
                           <?php } ?>
                           <?php if ($respuestaValida): ?>
                              <li> <a class="dropdown-item" href="<?php echo ENLACE_WEB; ?>mod_documentos_mercantiles/ajax/descargar.xml.php?tipo=<?php echo $Documento->nombre_clase ?>&id=<?php echo $Documento->id; ?>">Descargar Venta Respuesta <?php echo $Documento->referencia; ?> - XML</a>
                              </li>
                           <?php endif; ?>
                        <?php } ?>
               ...
               <?php if ($Documento->estado == 0) {   ?>
                  <a href="#" onclick="validar_albaran_venta()" class="btn btn-outline-primary _effect--ripple waves-effect">
                     <i class="fa fa-fw fa-check-circle-o" aria-hidden="true"></i> Aprobar <?php echo $Documento->documento_txt['singular']; ?>.
                  </a>
               <?php } ?>
...
<script>
   ...
   function abrir_email() {
      ...
      window.open('<?php echo ENLACE_WEB ?>mail_sys/mail/enviar_email.tpl.php?id=<?php echo $_REQUEST['fiche'] ?>&opcion=<?php echo $Documento->nombre_clase; ?>', 'Enviar Email Venta', opt);
   }

   function validar_albaran_venta() {
      ...
      Swal.fire({
         ...
         if (result.isConfirmed) {
            window.location.href = "<?php echo ENLACE_WEB; ?>dashboard.php?&accion=validar_albaran_venta&fiche=<?php echo $Documento->id; ?>&option_serie=" + codigo_serie_selected + "&option_bodega=" + codigo_bodega_selected;
   ...
   function anular_documento() {
      ...
      `,
         ...
         if (result.isConfirmed) {
            ...
            console.log('TODO: Pendiente de implementar')
            return;

            $.ajax({
                  method: "POST",
                  url: "<?php echo ENLACE_WEB; ?>mod_europa_albaran_venta/class/albaran.class.json.php",
                  data: {
                     documento: '<?php echo $Documento->id; ?>',
