...
<div class="middle-content container-fluid p-0">
    ...
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">Albaranes Venta </a></li>
...
<script>
    function cargar_listado_documento() {
        const ajaxoption = {
            url: '<?php echo ENLACE_WEB; ?>mod_europa_albaran_venta/ajax/albaran.venta.listado.ajax.php',
            type: 'GET'
        }
        ...
        options.initComplete = function() {
            ...
            api.columns().every(function() {
                ...
                if (headerText === 'Fecha') {
                    ...
                    const estados = $.ajax({
                        url: "<?php echo ENLACE_WEB; ?>mod_europa_albaran_venta/json/obtener_estado.php",
                        type: 'POST',
                        dataType: 'json'
                    })
        ...
        options.columns = [ 
            {
                ...
                render: function(data, type, row) {
                    return `
                        <a href="${ENLACE_WEB}albaran_venta/${row.id}">
...

            {
                ...
                render: function(data, type, row) {
                    ...
                    let htmlAcciones = `
                            ...
                            <a href="javascript:void(0);" onclick="${isBorrador ? 'return false;' : `generarPdf(${row.id}, 'Albaran_venta', 'Albarán Venta-${row.referencia}')`}" style="${linkStyle}" title="Descargar Albarán">
                                <i class="fa-solid fa-file-pdf fa-xl"></i>
                            </a>`
                    if (isBorrador) {
                        htmlAcciones +=
                            `
                            <a href="#" onclick="confirmar_eliminar_documento_mercantil(${row.id},  'Albaran_venta' ,'#style-3'); " title="Eliminar" style="color:red">
        ...
        let newButton = $('<button>')
            .html('<i class="fa-solid fa-plus"></i> Nueva Albarán Venta')
            .addClass('btn btn-primary')
            .attr("type", "button")
            .on('click', function() {
                window.location = '<?= ENLACE_WEB ?>nuevo_albaran_venta'
            });
