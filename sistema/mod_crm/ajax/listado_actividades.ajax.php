...
// WHERE 
$where ='';
 $where .=' AND ca.fk_oportunidad = '.$rowid.' ';
...
$where .= (!empty($_POST['filtro_usuario'])) ? " and fu.nombre like '%".$_POST['filtro_usuario']."%' " : "";
...
$consulta = "SELECT count(*) FROM fi_oportunidades_actividades ca 
left join fi_oportunidades c ON ca.fk_oportunidad = c.rowid 
left join diccionario_crm_actividades da ON ca.fk_diccionario_actividad = da.rowid
left join fi_terceros ft ON c.fk_tercero = ft.rowid
left join fi_usuarios fu ON ca.fk_usuario_asignado = fu.rowid
left join diccionario_crm_actividades_estado as de ON ca.fk_estado = de.rowid
where 1".$where;
...
$total_paginas       = ceil($num_total_registros / 15); 
...
$sql= "SELECT
             ...
             ca.fk_estado,
             ca.fecha_cierre,
             DATEDIFF(ca.vencimiento_fecha, IFNULL(ca.fecha_cierre, now()) )
              AS dias_vencimiento 
...
while ($data = $db->fetch(PDO::FETCH_OBJ)):
    ...
    if($data->fk_estado == 4)
    {
        ...
        $clase = 'success';
        $vencimiento .= 'Realizada: ('.(date("d/m/Y", strtotime($data->fecha_cierre))).')';
    }
}
    ...
    $tr.=
        '<tr id="tarea_'.$data->rowid.'" class="ng-scope '. ( ($_REQUEST['actividad']==$data->rowid) ? 'shake-row' :'' ).'" onclick="editarTarea('.$data->rowid.');" style="cursor:pointer;"   >
            <td><span>'.ucwords($data->actividad).'</span></td>
            <td><span>'.date('d/m/Y',strtotime($data->vencimiento_fecha)).'</span></td>
            <td><span class="badge bg-'.$clase.'">'.$vencimiento.'</span></td>
            <td><span>'.ucwords($data->usuario).'</span></td>
            <td><span>'.ucwords($data->estado).'</span></td>
        </tr>';
        $contador++;
endwhile;
...
<!-- <tr style="display:none;"><td><input type="hidden" name ="pagina_actualidad" value="<?php echo $_SESSION[$documentos_p]; ?>"></td></tr> -->
