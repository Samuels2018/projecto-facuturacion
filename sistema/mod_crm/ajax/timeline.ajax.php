...
if (isset($_GET['term'])) {
    ...
    FROM fi_oportunidades_actividades cpa 
    ...
    where cpa.fk_oportunidad = $term AND cpa.entidad = ".$_SESSION['Entidad']."  order by cpa.vencimiento_fecha  ASC";
    ...
    if ($count > 0) {
        while ($data = $db->fetch(PDO::FETCH_OBJ)) :
            ...
            //existe enlace de cotizacion
            if(intval($data->fk_cotizacion)>0)
            {
                 $row_array['fk_cotizacion'] = ENLACE_WEB.'ver_cotizacion/'.$data->fk_cotizacion;
            }else{
                if($data->tipo === 'tarea')
                {
                    $row_array['fk_cotizacion'] = ENLACE_WEB.'ver_oportunidad/'.$term.'/'.$data->rowid;
                }else{
                    $row_array['fk_cotizacion'] = '';
                }
            }
            ...
            if ($data->tipo == 'timeline') {
                ...
                $row_array['content'] = ' ' . $data->comentario;
            }
