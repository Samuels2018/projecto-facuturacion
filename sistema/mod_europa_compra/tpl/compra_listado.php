                            ...
                            <table id="style-3" class="table style-3 dt-table-hover p-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-left" scope="col">Serie Proveedor</th>
...
<script>
    function cargar_listado_documento() {
        const ajaxoption = {
            url: '<?php echo ENLACE_WEB; ?>mod_europa_compra/ajax/compra.listado.ajax.php',
            type: 'GET'
        }
        ...
        options.initComplete = function() {
            ...
            api.columns().every(function() {
                ...
                if (headerText === 'Fecha') {
                    ...
                    const estados = $.ajax({
                        url: "<?php echo ENLACE_WEB; ?>mod_europa_compra/json/obtener_estado.php",
                        type: 'POST',
                        dataType: 'json',
                    })
        ...
        options.columns = [
            {
                ...
                render: function(data, type, row) {
                    return `
                        <a href="${ENLACE_WEB}compra/${row.id}">
            ...
            {
                data: 'serie_proveedor',
                searchable: true,
                createdCell: function(td, cellData, rowData, row, col) {
                    $(td).attr('data-label', 'serie_proveedor');
                    $(td).addClass('text-left').addClass('py-0 px-3');
                },
                render: function(data, type, row) {
                    return `
                        <a href="${ENLACE_WEB}compra/${row.id}">
                            <i class="fa-regular fa-file-lines"></i> ${row.serie_proveedor}
                        </a>
                        `;
                }
            },
            ...
            },
            {
                ...
                searchable: true,
                createdCell: function(td, cellData, rowData, row, col) {
                    $(td).attr('data-label', 'Pagado').addClass('py-0 px-3 text-right');

                    // Agregar clase de color si estado_pagada es 1
                    if (rowData.estado_pagada == 1) {
                        $(td).addClass('text-success');
                    }
                },
                render: function(data, type, row) {
                    return row.pagado; // Valor de pagado
                }
            },

            {
                ...
                render: function(data, type, row) {
                    if(row.compra_referencia && row.compra_referencia.trim() != ""){
                        return `
...

            {
                ...
                searchable: true,
                createdCell: function(td, cellData, rowData, row, col) {
                    $(td).attr('data-label', 'Compra').addClass('py-0 px-3 text-right');
                },
                render: function(data, type, row) {
                    return `<a href ='<?php echo ENLACE_WEB; ?>compra/${row.id}' ><span class="p-2 badge badge-light-${row.estado_final_class}">${row.estado_final} </span></a>`;
                }
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    let htmlAcciones = `
                                ...
                                <a href="<?php echo ENLACE_WEB; ?>mod_documentos_mercantiles/ajax/descargar.comprobante.php?tipo=compra&id=${row.id}" id="download-xml" style="${linkStyleXML}" ${!archivoExistente || isBorrador ? 'onclick="return false;"' : ''} title="Descargar XML">
                                    <i class="fa-regular fa-file-code fa-xl" style="opacity: 0.3;"></i>
                                </a>
                                <a href="javascript:void(0);" onclick="${isBorrador ? 'return false;' : `generarPdf(${row.id}, 'compra', 'Compra-${row.referencia}')`}" title="Descargar Compra">
                                    <i class="fa-solid fa-file-pdf fa-xl"></i>
                                </a>
                                <a href="<?php echo ENLACE_WEB; ?>mod_documentos_mercantiles/ajax/descargar.xml.php?tipo=compra&id=${row.id}" id="download-xml" style="${linkStyleXMLT}" ${!xmlExists || isBorrador ? 'onclick="return false;"' : ''} title="Descargar XML Huella">
                                    <i class="fa-solid fa-stamp fa-xl" style="opacity: 0.3;"></i>
                                </a>`
                    if (isBorrador) {
                        htmlAcciones +=
                            `
                                <a href="#" onclick="confirmar_eliminar(${row.id}, ${row.estado}, 'compra'); " id="eliminar" title="Eliminar" style="color:red">
        ...
        let newButton = $('<button>')
            .html('<i class="fa-solid fa-plus"></i> Nueva Compra')
            .addClass('btn btn-primary')
            .attr("type", "button")
            .on('click', function() {
                window.location = '<?= ENLACE_WEB ?>nueva_compra'
        });
