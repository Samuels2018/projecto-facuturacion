...
$Factura = new Factura($dbh, $entidad);
...
$mapaColumnas = array(
    ...
    "envio_correo_factura" => "a.envio_correo_factura",
    "envio_correo_factura_fecha" => "a.envio_correo_factura_fecha",
    "envio_correo_factura_correo" => "a.envio_correo_factura_correo",
...
$sqlstr = "SELECT 
    ...
    a.envio_correo_factura, 
    a.envio_correo_factura_fecha,
    a.envio_correo_factura_correo,  
    CONCAT(u.nombre, ' ', u.apellidos) AS usuario_crear,
    d.etiqueta AS estado_final,
    d.class as estado_final_class,  
    dv.etiqueta AS estado_verifactu,
    dv.class AS estado_verifactu_color, 
    CASE 
...
LEFT JOIN ".DB_NAME_UTILIDADES_APOYO.".diccionario_factura_europa_diccionario d ON a.estado = d.rowid
LEFT JOIN ".DB_NAME_UTILIDADES_APOYO.".diccionario_factura_europa_verifactu_diccionario dv ON a.estado_hacienda = dv.rowid
...
// Verifica si hay otros criterios de búsqueda específicos para las columnas
$otherFilters = false;
...
$sqlstrFilter = "SELECT COUNT(*) FROM fi_europa_facturas a WHERE a.entidad = :entidad";
...
foreach ($Records as $row) {
    ...
    $data[] = array(
        ...
        "envio_correo_factura" => $row['envio_correo_factura'],
        "envio_correo_factura_fecha" => $row['envio_correo_factura_fecha'],
        "envio_correo_factura_correo" => $row['envio_correo_factura_correo'],
        "total" => $row['total'],
        ...
        "estado_verifactu" => $row['estado_verifactu'],
        "estado_verifactu_color" => $row['estado_verifactu_color']
    );
}
...
$response = array(
    ...
    "data" => $data,
    "archivo_existente" => $fileExists ? true : false,
    "xmlexists" => $xmlexists,
    'sql' => $sqlstr,
);
