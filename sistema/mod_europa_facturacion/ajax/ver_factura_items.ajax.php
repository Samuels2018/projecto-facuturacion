...
$Documento2  = new Factura($dbh, $_SESSION['Entidad']);
if ($document_id != '') {
...
if (
    ...
    $Documento2->cantidad           =  numero_simple_coma($_POST['cantidad']);
    $Documento2->precio_unitario    =  numero_simple_coma($_POST['precio_unitario']);
    $Documento2->label              =   (empty($_POST['nombre'])) ? ' ' : $_POST['nombre'];
    $Documento2->fk_producto        = $_POST['fk_producto'];
    $Documento2->descuento          = numero_simple_coma($_POST['descuento']);
    $Documento2->descuento_tipo     = $_POST['descuento_tipo'];
    $Documento2->tipo_impuesto          = numero_simple_coma($_POST['impuesto']);
...
while ($obj = $db->fetch(PDO::FETCH_ASSOC)) {
    ...
    $etiqueta_html = '<span style="display:block; overflow:visible; text-overflow:clip; white-space:normal; max-width:100%;">'.nl2br($obj['label']).'</span>';
    if ($obj['fk_producto'] > 0) {
        $simbolo = '<a class="producto_' . $obj['fk_producto'] . '" id="label_etiqueta_' . $obj['rowid'] . '" > <i class="fa fa-fw fa  fa-check-circle"></i>' . $etiqueta_html . ' </a>';
        '  ';
    } else {
        $simbolo = ' <i id="label_etiqueta_' . $obj['rowid'] . '" class="fa fa-fw fa fa-puzzle-piece"></i> <br/>' . $etiqueta_html . '';
    }
    ...
    $input_cantidad .= '<label id="cantidad_' . $obj['rowid'] . '">' . numero_simple($obj['cantidad'], 2) . '</label>';
