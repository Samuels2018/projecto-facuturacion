...
require_once(ENLACE_SERVIDOR . "mod_europa_pedido/object/pedido.object.php");
...
$Factura = new Pedido($dbh, $entidad);
...
$sqlstr = "SELECT 
    ...
    d.etiqueta as estado_txt, d.class as estado_class 


FROM fi_europa_pedidos  a
LEFT JOIN fi_usuarios u ON a.fk_usuario_crear = u.rowid
LEFT JOIN fi_terceros t ON a.fk_tercero = t.rowid
    LEFT JOIN " . DB_NAME_UTILIDADES_APOYO . ".diccionario_pedidos_europa_diccionario d ON a.estado = d.rowid
...
$sqlstrTotal = "SELECT COUNT(*) FROM fi_europa_pedidos a WHERE a.entidad = :entidad";
...
foreach ($Records as $row) {
    ...
    $data[] = array(
        ...
        "envio_correo_pedido" => $row['envio_correo_pedido'],
        "envio_correo_pedido_fecha" => $row['envio_correo_pedido_fecha'],
        "envio_correo_pedido_correo" => $row['envio_correo_pedido_correo'],
        "total" => $row['total'],
