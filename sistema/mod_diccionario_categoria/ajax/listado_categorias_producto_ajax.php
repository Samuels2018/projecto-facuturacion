...
$mapaColumnas = array(
    "ID" => "a.rowid",
    "label" => "COALESCE(c2.label, a.label)",  // Consideramos tanto la categoría como la subcategoría
    "estado" => "a.activo",
    "sub_categoria" => "(CASE WHEN a.fk_parent IS NOT NULL THEN a.label ELSE '' END)",
);
...
// Nueva consulta que refleja la estructura de categorías y subcategorías
$sqlstr = "
    SELECT 
        a.rowid, 
        a.activo,
        COALESCE(c2.label, a.label) AS Categoria, 
        CASE WHEN a.fk_parent IS NOT NULL THEN a.label ELSE '' END AS Subcategoria, 
        CASE WHEN a.borrado = 0 THEN 'Activa' ELSE 'Inactiva' END AS Estado 
    FROM 
        diccionario_categorias a 
    LEFT JOIN 
        diccionario_categorias c2 ON a.fk_parent = c2.rowid
    WHERE 1 
";
...
$sqlstr .= ' AND a.borrado = 0'; // Solo muestra los registros no borrados

// Ordena por la categoría y subcategoría
$sqlstr .= " ORDER BY Categoria, Subcategoria";
