  ...
  <script>
      function cargar_tabla_direccion() {
          vtabla = $('#style-3').DataTable({
              ...
              "stripeClasses": [],
              'ajax': {
                  'url': '<?php echo ENLACE_WEB; ?>mod_direcciones/ajax/listado_direcciones_ajax.php',
                  'Method': 'POST'
              },
              retrieve: true,
              deferRender: true,
              ...
              columns: [{
                      ...
                      render: function(data, type, row) {
                          ...
                          return `<a onclick="ver_direccion(${row.ID});" href="#">${row.ID}</a>`
                      }
                  },
                  {
                      data: 'descripcion',
                      searchable: true,
                      createdCell: function(td, cellData, rowData, row, col) {
                          $(td).addClass('text-center').attr('data-label', 'descripcion');
                      },
                      render: function(data, type, row) {
                          return `<a onclick="ver_direccion(${row.ID});" href="#">${data}</a>`
                      }
...

                  {
                      ...
                      searchable: true,
                      createdCell: function(td, cellData, rowData, row, col) {
                          $(td).addClass('text-center').attr('data-label', 'poblacion');
                      },
                      render: function(data, type, row) {
                          return `<a onclick="ver_direccion(${row.ID});" href="#">${data}</a>`
                      }
  ...
  <script>
      $(document).ready(function() {
          ...
          $("#direccion_listado").addClass('active');
          ...
          let newButton = $('<button>')
              .html('<i class="fa-solid fa-plus"></i> Nueva dirección')
  ...
  <script type="text/javascript">
    ...
    //   let marker = null;
      ...
      function ver_direccion(int = null) {
          // Preparar la petición AJAX
          $.ajax({
              method: "POST",
      ...
      function actualizar_direccion(int) {
          ...
          $.ajax({
              ...
              if (mensaje.exito === 1) {
                  add_notification({
                      text: 'Dirección actualizado exitosamente',
                      actionTextColor: '#fff',
                      backgroundColor: '#00ab55',
                      dismissText: 'Cerrar'
                  });
      ...
      function borrar_direccion($id) {
          // Preparar el mensaje para el snackbar
          var message = "Está seguro(a) que desea eliminar la Dirección? ";
      ...
      document.getElementById('nueva_direccion').addEventListener('hidden.bs.modal', function() {
