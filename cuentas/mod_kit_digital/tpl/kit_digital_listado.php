...
// Obtener estados de Kit Digital
$estados_kit_digital = $kitDigital->listar_estados_kitdigital();
// Convertir los estados en JSON para pasarlos a JavaScript
$estados_kit_json = json_encode($estados_kit_digital);
...
<style type="text/css">
  ...
  <div class="middle-content container-xxl p-0">
      ...
      <div class="row layout-top-spacing">
          ...
          <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
              <div class="row my-4">
                  <div class="col-md-3">
                      <a href="<?php echo ENLACE_WEB_CUENTAS.'nuevo_kit_digital/'; ?>" class="btn btn-primary">Nuevo</a>
                  </div>
                          ...
                          <table id="style-3" class="table style-3 dt-table-hover">
                              <thead>
                                <tr>
                                    ...
                                    <th scope="col">Aplica KitDigital</th>
                                    <th scope="col">Estado KitDigital</th>
  ...
  <script>
      function cargar_tabla_crm() {
        ...
        var estadosKitDigital = <?php echo $estados_kit_json; ?>;
        var enlace_web_cuentas = '<?php ENLACE_WEB_CUENTAS; ?>';
    ...
    "stripeClasses": [],
    'ajax': {
        'url': '<?php echo ENLACE_WEB_CUENTAS; ?>mod_kit_digital/ajax/listado.kit.ajax.php',
        'Method': 'POST'
    },
    retrieve: true,
    deferRender: true,
    ...
    "initComplete": function() {
        this.api().columns().every(function(col) {
            ...
            var inputContainer = $('<div>').css({ 'width': '100%', 'height': '100%', 'position': 'relative' }).appendTo(header);

            if (headerText === 'Tipo') {
                $('<select class="form-select"><option value="">Todos</option><option value="juridica">Jurídica</option><option value="fisica">Física</option></select>')
            ...
            else if (headerText === 'Aplica KitDigital') {
                $('<select class="form-select"><option value="">Todos</option><option value="1">SI</option><option value="0">NO</option></select>')
                    .appendTo(inputContainer)
                    .on('change', function() {
                        var val = $.fn.dataTable.util.escapeRegex($(this).val());
                        column.search(val).draw();
            ...
            else if (headerText === 'Estado KitDigital') {
                var estadoSelect = $('<select class="form-select"><option value="">Seleccione el estado</option></select>');
                estadosKitDigital.forEach(function(estado) {
                    estadoSelect.append(`<option value="${estado.rowid}">${estado.etiqueta}</option>`);
                });
                estadoSelect.appendTo(inputContainer).on('change', function() {
                    var val = $.fn.dataTable.util.escapeRegex($(this).val());
                    column.search(val).draw();
                });
            } else {
    ...
    'columns': [
        {
            ...
            render: function(data, type, row) {
                return `<a href="<?php echo ENLACE_WEB_CUENTAS; ?>editar_kit_digital/${row.ID}">${data}</a>`;
            }
        },
        {
            ...
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-left').attr('data-label', 'Nombre Completo');
            },
            render: function(data, type, row) {
                return `<a href="<?php echo ENLACE_WEB_CUENTAS; ?>editar_kit_digital/${row.ID}">${data}</a>`;
            }
        },
        {
            ...
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-left').attr('data-label', 'Tipo');
            }
        },
        {
            data: 'Cedula',
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-left').attr('data-label', 'Cédula');
            }
        },
        {
            data: 'Telefono',
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-left').attr('data-label', 'Teléfono');
            }
        },
        {
            data: 'Poblacion',
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-left').attr('data-label', 'Población');
            }
        },
        {
            data: 'Aplica KitDigital',
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-center').attr('data-label', 'Aplica KitDigital');
            }
        },
        {
            data: 'Estado KitDigital',
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-center').attr('data-label', 'Estado KitDigital');
            }
        },
        {
            data: 'Monto',
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-right').attr('data-label', 'Monto');
            }
        },
        {
            data: 'Cobrado',
            searchable: true,
            createdCell: function(td) {
                $(td).addClass('text-right').attr('data-label', 'Cobrado');
            }
        }
    ]
});
